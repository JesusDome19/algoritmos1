...
class BinaryTree:
    ...
    def postorden_superheros(self):
        def __postorden_superheros(root):
            if root is not None:
                __postorden_superheros(root.right)
                if root.other_value.get('is_hero') is True:
                    print(root.value)
                __postorden_superheros(root.left)

        if self.root is not None:
            __postorden_superheros(self.root)
    ...
    def inorden_dioses_start_with(self, start):
        def __inorden_dioses_start_with(root, start):
            if root is not None:
                __inorden_dioses_start_with(root.left, start)
                if root.value.startswith(start):
                    print(f"  {root.other_value}")
                __inorden_dioses_start_with(root.right, start)

        if self.root is not None:
            __inorden_dioses_start_with(self.root, start)
    ...
    def criatura_search(self, search_value):
        def __criatura_search(root, search_value):
            if root is not None:
                __criatura_search(root.left, search_value)
                if root.value.startswith(search_value):
                    print(f"{root.value}: {root.other_value}")
                __criatura_search(root.right, search_value)

        if self.root is not None:
            __criatura_search(self.root, search_value)

    def search_dioses(self, search_value):
        def __search_dioses(root, search_value):
            if root is not None:
                __search_dioses(root.left, search_value)
                if root.other_value[1]["capturada"].startswith(search_value):
                    print(f"{root.value}: {root.other_value}")
                __search_dioses(root.right, search_value)

        if self.root is not None:
            __search_dioses(self.root, search_value)
