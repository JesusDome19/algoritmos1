...
class BinaryTree:
    ...
    def search(self, key):
        def __search(root, key):
            if root is not None:
                ...
                print(f'El pokemon no se encotro ')
        aux = None
        if self.root is not None:
            aux = __search(self.root, key)
        return aux

    def search_number(self, key):
            def __search_number(root, key):
                if root is not None:
                    if root.value == key:
                        print(f'{root.other_value}')
                        return root
                        ...
                        # print(f'buscalo a la izquierda de {root.value}')
                        return __search_number(root.left, key)
                    else:
                        # print(f'buscalo a la derecha de {root.value}')
                        return __search_number(root.right, key)
                else:
                    print(f'El pokemon de numero {key} no se encontro ')
            aux = None
            if self.root is not None:
                aux = __search_number(self.root, key)
            return aux
    ...
    def contar_pokemons(self, search_value):
        def __contar_pokemons(root, search_value):
            counter = 0
            if root is not None:
                if root.value in search_value:
                    counter = 1
                counter += __contar_pokemons(root.left, search_value)
                counter += __contar_pokemons(root.right, search_value)
            return counter

        return __contar_pokemons(self.root, search_value)
    ...
    def proximity_search(self, search_value):
        found = False  # Indicador para saber si encontramos el valor

        def __proximity_search(root, search_value):
            nonlocal found  # Permite modificar 'found' dentro de la función interna
            if root is not None:
                __proximity_search(root.left, search_value)
                # Verifica si root.value es una cadena antes de usar 'startswith'
                if isinstance(root.value, str) and root.value.startswith(search_value):
                    print(f"{root.value} -- {root.other_value}")
        ...
        if self.root is not None:
            ...
            if not found:
                print(f"No se encontro el pokemon de nombre {search_value}")
                
                        
    def proximity_search_inicial(self, search_value):
        found = False  # Indicador para saber si encontramos el valor

        def __proximity_search_inicial(root, search_value):
            nonlocal found  # Permite modificar 'found' dentro de la función interna
            if root is not None:
                __proximity_search_inicial(root.left, search_value)
                # Verifica si root.value es una cadena antes de usar 'startswith'
                if isinstance(root.value, str) and root.value.startswith(search_value):
                    print(f"{root.value} -- {root.other_value}")
                    found = True  # Marcamos como encontrado
                __proximity_search_inicial(root.right, search_value)

        if self.root is not None:
            __proximity_search_inicial(self.root, search_value)
            if not found:
                print(f"No se encontro el pokemon con las inciales {search_value}")
    ...
    def proximity_search_tipo(self, search_value):
        ...
        if self.root is not None:
            ...
            if not found:
                print(f"No se encontro el pokemon del tipo {search_value} ")
